    ...
    methods: {
      ...
      deletePur(data) {
        this.$confirm('此操作将永久删除订单号为【' + data.oid + '】的订单, 是否继续?', '提示', {
          ...
          this.deleteRequest("/mat/in/" + data.id).then(resp => {
            if (resp) {
              this.initPurs();
            }
          })
      ...
      doAddPur() {
        if (this.order.id) {
          this.$refs['purForm'].validate(valid => {
            if (valid) {
              this.putRequest("/mat/in/", this.order).then(resp => {
                if (resp) {
          ...
          this.$refs['purForm'].validate(valid => {
            if (valid) {
              this.postRequest("/mat/in/", this.order).then(resp => {
                if (resp) {
                  this.dialogVisible = false;
                  this.initPurs();
                }
      ...
      initData() {
        ...
        if (!window.sessionStorage.getItem("materiel")) {
          this.getRequest('/mat/in/materiel/').then(resp => {
            if (resp) {
              this.states = resp;
              window.sessionStorage.setItem("materiel", JSON.stringify(resp));
            }
          })
        } else {
          this.materiel = JSON.parse(window.sessionStorage.getItem("materiel"));
        }
      },
      ...
      initPurs(type) {
        this.loading = true;
        let url = '/mat/in/?page=' + this.page + '&size=' + this.size;
        if (type && type == 'advanced') {
          ...
          if (this.searchValue.materiel) {
            url += '&materiel=' + this.searchValue.materiel;
          }
          if (this.searchValue.dateScope) {
            url += '&dateScope=' + this.searchValue.dateScope;
          }
        } else {
          url += "&materiel=" + this.keyword;
        }
